...
// image size
let HRToastImageViewWidth :  CGFloat  = 35.0
let HRToastImageViewHeight:  CGFloat  = 35.0

// label setting
let HRToastMaxWidth       :  CGFloat  = 0.8      // 80% of parent view width
let HRToastMaxHeight      :  CGFloat  = 0.8
let HRToastFontSize       :  CGFloat  = 22.0
...
let HRToastCornerRadius   : CGFloat   = 0
    ...
    func centerPointForPosition(position: AnyObject, toast: UIView) -> CGPoint {
        if position is String {
            ...
            if position.lowercaseString == HRToastPositionTop {
                return CGPointMake(viewSize.width/2, toastSize.height/2 + 3 * HRToastVerticalMargin)
            } else if position.lowercaseString == HRToastPositionDefault {
                return CGPointMake(viewSize.width/2, viewSize.height - toastSize.height/2 - 3 * HRToastVerticalMargin)
